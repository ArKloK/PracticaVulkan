...
//
// FUNCIÓN: CAVulkanState::windowResized()
//
// PROPÓSITO: Actualiza el estado de Vulkan al redimensionar el
// 	          tamaño de la ventana
//
void CAVulkanState::windowResized()
{
	glfwGetFramebufferSize(window, &wWidth, &wHeight);
...
//
// FUNCIÓN: CAVulkanState::windowResized(GLFWwindow* window)
//
// PROPÓSITO: Actualiza el estado de Vulkan al redimensionar el
// 	          tamaño de la ventana
//
...
{
	this->model = model;
	this->model->aspect_ratio((1.0 * wWidth) / wHeight);
	fillCommandBuffers();
}
...
//
uint32_t CAVulkanState::waitForNextImage()
{
...
//
// FUNCIÓN: CAVulkanState::createBuffer(VkBuffer* buffer, VkDeviceMemory* deviceMemory, size_t size, VkBufferUsageFlagBits usage)
//
// PROPÓSITO: Crea un buffer con su memoria asociada
//
void CAVulkanState::createBuffer(VkBuffer* buffer, VkDeviceMemory* deviceMemory, size_t size, VkBufferUsageFlagBits usage)
{
...
//
// FUNCIÓN:CAVulkanState::createDescriptorSets(VkDescriptorPool* descriptorPool, std::vector<VkDescriptorSet> *descriptorSets, VkBuffer** buffers, size_t* size, int bufferCount)
//
// PROPÓSITO: Crea los conjuntos de descriptores asociados a los buffers
//
void CAVulkanState::createDescriptorSets(VkDescriptorPool* descriptorPool, std::vector<VkDescriptorSet> *descriptorSets, VkBuffer** buffers, size_t* size, int bufferCount)
{
	VkDescriptorPoolSize poolSize1 = {};
	poolSize1.type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
	poolSize1.descriptorCount = imageCount;
	...
	poolSize3.descriptorCount = imageCount;

	VkDescriptorPoolSize poolSizes[] = { poolSize1, poolSize2, poolSize3 };
	...
	for (size_t i = 0; i < imageCount; i++)
	{
		...
		{
			buffersInfo[j] = {};
			buffersInfo[j].buffer = buffers[j][i];
			buffersInfo[j].offset = 0;
			buffersInfo[j].range = size[j];
		}
		...
		descriptorWrite.descriptorCount = bufferCount;
