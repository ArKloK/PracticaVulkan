...
//
// FUNCIÓN: CAApplication::draw()
//
// PROPÓSITO: Lanza la generación del dibujo
//
void CAApplication::draw()
{
	uint32_t imageIndex = vulkan->waitForNextImage();
...
{
	CAApplication* app = (CAApplication*)glfwGetWindowUserPointer(window);
	app->vulkan->windowResized();
	if (height != 0)
	{
		double aspect = (double)app->vulkan->wWidth / (double)app->vulkan->wHeight;
